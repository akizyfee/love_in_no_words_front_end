...
const ptTemp = reactive({
  iceTypes: ['正常冰', '少冰', '微冰', '去冰', '熱'],
  sugarTypes: ['全糖', '七分', '半糖', '三分', '無糖'],
...
<template>
  <p class="text-4xl">
    因為這裡畫面原本是 bootstrap 所以搬過來會跑但是就不花大把時間回頭修了，功能還是能跑qqq
  </p>
      ...
      <div class="col-md-4">
        <div class="list-group">
          <a
            ...
            v-for="item in ptTemp.products"
            :key="item.name"
            @click.prevent="selectPt(item)"
            :class="{ active: selectItem.name === item.name }"
          >
        ...
        <div class="card mb-2">
          ...
          <div class="card-body px-4">
            ...
            <div class="mb-3">
              ...
              <div
                v-for="(icetype, key) in ptTemp.iceTypes"
                :key="key"
                class="form-check form-check-inline"
              >
                <input
                  v-model="selectItem.iceLevel"
                  :value="icetype"
                  :id="'icetype' + key"
                  type="radio"
                  class="form-check-input"
                />
                <label class="form-check-label" :for="'icetype' + key">{{ icetype }}</label>
              </div>
            </div>
            <div class="mb-3">
              <label for="productNum" class="form-label d-block">甜度*</label>
              <div
                v-for="(sugartype, key) in ptTemp.sugarTypes"
                :key="key"
                class="form-check form-check-inline"
              >
                <input
                  v-model="selectItem.sugarLevel"
                  :value="sugartype"
                  :id="'sugartype' + key"
                  type="radio"
                  class="form-check-input"
                />
                <label class="form-check-label" :for="'sugartype' + key">{{ sugartype }}</label>
              </div>
            </div>
            <div class="mb-3">
              <label for="productNum" class="form-label d-block">加料</label>
              <div
                v-for="(topping, key) in ptTemp.toppingsType"
                :key="'topping' + key"
                class="form-check form-check-inline"
              >
                <input
            ...
            <div class="d-flex gap-2">
              <button @click="resetOrder()" class="btn btn-outline-primary w-100" type="button">
                取消
              </button>
              <button @click="addToOder(selectItem)" class="btn btn-dark w-100" type="button">
                加入
              </button>
            </div>
          </div>
            ...
            <table class="table">
              ...
              <tbody>
                <tr v-for="(item, key) in orderList" :key="'order' + key">
                  ...
                  <td>{{ item.iceLevel.toString() }}</td>
                  <td>{{ item.sugarLevel.toString() }}</td>
  ...
  <!-- <div class="bg-light p-3 mt-3">
    <div class="bg-white p-3 d-flex flex-column" style="min-height: 450px;">
      ...
      <p class="mt-3 mb-1">訂單成立時間：2021-01-12 03:37:01</p>
      <p class="mb-1">餐點數： 1</p>
      <p class="mb-1">付款狀態：未付款</p>
      <p class="text-end mt-auto">共 NT$ 70 元</p>
    </div>
  </div> -->
</template>
